...
[router]
enabled = true
classifier_url = "http://localhost:11434/v1"    # Ollama 端点
classifier_model = "qwen2.5:3b"                 # 用于分类的轻量模型
classifier_api_key = ""                          # Ollama 不需要
...
<Aside type="note">
  智能路由需要本地 LLM 服务（如 Ollama）进行意图分类。分类器模型应选择小而快的 — `qwen2.5:3b` 效果很好。
</Aside>
