...
5. `~/.config/claudex/config.toml`（XDG — **优先于**平台特定路径）
6. 平台特定路径（macOS 为 `~/Library/Application Support/claudex/config.toml` 等）

<Aside type="note">
  XDG 路径（`~/.config/claudex/config.toml`）现在优先于平台特定路径。如果所有位置都不存在配置文件，创建默认配置时使用 XDG 路径。
</Aside>
...
<Aside type="tip">
  此类型主要用于 OAuth 订阅。详细的 Codex CLI 配置步骤请参见 [Provider 配置指南](/claudex/zh-cn/guides/provider-setup/)。
</Aside>
...
每个提供商的详细配置步骤（包括 API 密钥获取链接和 OAuth 流程），请参见 [Provider 配置指南](/claudex/zh-cn/guides/provider-setup/)。
