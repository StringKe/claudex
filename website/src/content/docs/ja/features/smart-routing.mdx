...
1. ユーザーが `claudex run auto` を実行する
2. Claude Code がプロキシにリクエストを送信する
3. プロキシがユーザーのメッセージを抽出し、ローカル分類器 (例: Ollama) に送信する
4. 分類器がインテントカテゴリ (code、analysis、creative、search、math) を返す
5. ルーティングルールがインテントを最適なプロバイダープロファイルにマッピングする
...
| `math` | 数学的・論理的タスク | 計算量の計算、正確性の証明 |

<Aside type="note">
  スマートルーティングはインテント分類のために LLM サービスが必要です。既存のプロファイルの `base_url` と `api_key` を再利用します。分類器モデルは小さくて高速なものが望ましいです。`qwen2.5:3b` が適しています。
</Aside>
