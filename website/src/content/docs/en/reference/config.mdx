...
| `classifier_url` | string | `"http://localhost:11434/v1"` | OpenAI-compatible endpoint for intent classification |
| `classifier_model` | string | `"qwen2.5:3b"` | Model to use for classification |
| `classifier_api_key` | string | `""` | API key for classifier (empty for local services) |
...
| `summarizer_url` | string | — | LLM endpoint for generating summaries |
| `summarizer_model` | string | — | Model to use for summarization |
...
| `embedding_url` | string | — | Endpoint for embedding generation |
