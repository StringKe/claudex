...
<Aside type="caution">
  컨텍스트 엔진 기능은 요약 및 임베딩 생성을 위한 LLM 서비스가 필요합니다. 기존 프로파일(로컬 Ollama, OpenRouter 같은 클라우드 제공자 등)을 사용할 수 있습니다.
</Aside>
...
2. 토큰이 `threshold_tokens`를 초과하면, `keep_recent` 이전의 오래된 메시지가 요약으로 대체됨
3. 설정된 로컬 LLM이 요약 생성
4. 압축된 대화가 제공자에게 전달됨

## 프로파일 간 공유

동일 세션 내에서 서로 다른 제공자 프로파일 간에 컨텍스트를 공유합니다.
...
작업 중간에 제공자를 전환할 때 유용합니다 — 이전 상호작용의 관련 컨텍스트가 자동으로 포함됩니다.
...
2. 각 요청에 대해 사용자 메시지가 임베딩되고 인덱스와 비교됨
3. 가장 관련성이 높은 상위 k개의 청크가 요청에 추가 컨텍스트로 주입됨
4. 제공자가 코드베이스에 대한 더 풍부한 컨텍스트를 받음
